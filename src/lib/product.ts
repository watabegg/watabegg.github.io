...
export function parseProductBody(
	...
	if (!trimmed) {
		throw new Error(
			`products/${id ?? 'unknown'} の本文が空です。'---' で区切ったセクションを記述してください。`,
		)
	}
	...
	if (sections.length < 7) {
		throw new Error(
			`products/${id ?? 'unknown'} の本文が ${sections.length} セクションしかありません。8つのセクション (概要, サマリ, 期間, 使用技術, 業務領域, 工夫点, 成果物, 余談) を '---' で区切ってください。余談は空でも構いませんが区切りは必要です。`,
		)
	}
	...
	if (!overview) {
		throw new Error(`products/${id ?? 'unknown'} の概要セクションが空です。`)
	}

	if (!overviewSummary) {
		throw new Error(
			`products/${id ?? 'unknown'} の概要サマリセクションが空です。スキルシート用の短いサマリを記載してください。`,
		)
	}
	...
	if (!PERIOD_PATTERN.test(periodValue)) {
		throw new Error(
			`products/${id ?? 'unknown'} の期間セクションは "YYYY-MM-DD/YYYY-MM-DD" もしくは "YYYY-MM-DD/present" 形式で記述してください。`,
		)
	}
	...
	return {
		...
		overviewSummary,
		period: periodValue,
		techStack: parseTechStack(techSection, id),
		responsibilities: parseLineItems(responsibilitiesSection),
		improvements: parseLineItems(improvementsSection),
		deliverables: parseDeliverables(deliverablesSection, id),
		extra: extraSection || undefined,
	} satisfies ProductContentSections
}

function parseTechStack(section: string, id?: string): ProductTechStackItem[] {
	const lines = parseLineItems(section)
	if (lines.length === 0) {
		throw new Error(
			`products/${id ?? 'unknown'} の使用技術セクションに項目がありません。`,
		)
	}
	...
	return lines.map((line) => {
		...
		if (!name) {
			throw new Error(
				`products/${id ?? 'unknown'} の使用技術セクションで名前が空の項目があります。`,
			)
		}
...
function parseDeliverables(section: string, id?: string): ProductDeliverable[] {
	const lines = parseLineItems(section)
	return lines.map((line) => {
		...
		if (parts.length >= 2) {
			...
			if (!label || !url) {
				throw new Error(
					`products/${id ?? 'unknown'} の成果物セクションのフォーマットが不正です。「ラベル | https://example.com」のように記載してください。`,
				)
			}
		...
		throw new Error(
			`products/${id ?? 'unknown'} の成果物セクションにURLが含まれていません。「ラベル | https://example.com」またはMarkdownリンクで記載してください。`,
		)
	})
}
