...
export async function fetchZennItems(perPage: number = 20): Promise<ZennResponse> {
  ...
  try {
    ...
    return {
      ...
      total: Number.isFinite(parsed.data.total_count) ? parsed.data.total_count ?? null : null,
    };
  } catch {
